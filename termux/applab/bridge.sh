...
repo_json_fields() {
  if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    if [ -f "$SHARED_DIR/patches/patch.sh" ]; then
      HAS_PATCH_FILE="true"
    else
    ...
    python3 - \
      "$REPO_PATH" "$REPO_NAME" "$BRANCH" "$DIRTY" "$CHANGED_FILES" \
      "$STAGED_FILES" "$UNSTAGED_FILES" "$UNTRACKED_FILES" "$AHEAD" "$BEHIND" \
      "$CURRENT_COMMIT" "$CURRENT_COMMIT_MESSAGE" "$UPSTREAM" "$REMOTE_URL" "$HAS_PATCH_FILE" <<'PY'
import json
import sys
(
    repo_path,
...
write_result() {
  ...
  python3 - "$RESULTS_DIR/latest_result.json" "$EXTRA_JSON" <<PY
import json, sys, os
path = sys.argv[1]
extra_text = sys.argv[2] if len(sys.argv) > 2 else "{}"
data = {
  "schema_version": 1,
  "run_id": "$RUN_ID",
  "action": "$ACTION",
